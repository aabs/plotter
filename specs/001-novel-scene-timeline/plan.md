...
**Project Type**: NuGet-distributed CLI tool with an optional interactive TUI and derived exporters.

**Performance Goals**: Chronological query over 1,000 scenes under 2 seconds on a typical personal computer; exports and audits should operate from one loaded workspace snapshot.

**Constraints**: TOML remains canonical; current-folder resolution is the default; stable plain-text output; machine formats must preserve equivalent results; no external service required; TUI is a projection over shared query services; central package management; warnings as errors/analyzers; immutable-first domain models; 100% production coverage; property-based tests by default.
...
Research is captured in [research.md](research.md). Key decisions are .NET 10/C# 14, Spectre.Console, TOML current-folder resolution, shared projections, central package management, FsCheck property testing, and staged export breadth.
...
5. Implement stable text/JSON/CSV/SARIF/Markdown projections and export boundaries.
6. Implement Spectre.Console command registration and the TUI over the shared queries.
7. Add NuGet tool packaging and help/quickstart documentation.
8. Drive each slice test-first with FsCheck properties and enforce coverage/analyzer gates.
